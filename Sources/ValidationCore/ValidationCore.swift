...
public struct ValidationCore {
    ...
    // MARK: - Public API
        ...
        public mutating func validateQrCode(_ qrView: UIView, _ completionHandler: @escaping (ValidationResult) -> Void) {
    ...
    public func validate(encodedData: String, validationClock: Date, _ completionHandler: @escaping (ValidationResult) -> Void) {
        let cwtResult = decodeCwtAndCose(encodedData: encodedData)

        if case let .failure(error) = cwtResult {
        ...
        if case let .success(decodingResult) = cwtResult {
            trustlistService.key(for: decodingResult.keyId, cwt: decodingResult.cwt, keyType: decodingResult.cwt.euHealthCert!.type) { result in
                ...
                case let .failure(error):
                    completionHandler(ValidationResult(isValid: false, metaInformation: MetaInfo(from: decodingResult.cwt), greenpass: decodingResult.cwt.euHealthCert!, error: error))
                }
            }
        }
    }
...
// MARK: - QrCodeReceiver
    ...
    extension ValidationCore: QrCodeReceiver {
        ...
        public func onQrCodeResult(_ result: String?) {
            ...
            validate(encodedData: result, validationClock: Date(), completionHandler)
        }
    }
#endif

struct ValidationClockDateService: DateService {
    let now: Date
