...
public enum CryptoService {
    public static func generateSymmetricKey(for alias: String, completionHandler: @escaping (ValidationError?) -> Void) throws {
    ...
    public static func createKeyAndEncrypt(data: Data, with keyAlias: String, completionHandler: @escaping (Result<Data, ValidationError>) -> Void) {
    ...
    public static func decrypt(ciphertext: Data, with keyAlias: String, completionHandler: @escaping (Result<Data, ValidationError>) -> Void) {
    ...
    private static func authenticate(_ completionHandler: @escaping (LAContext?, Error?) -> Void) {
    ...
    private static func createKey(for alias: String, with context: LAContext) -> SymmetricKey? {
        ...
        guard (try? storeKey(key, alias: alias, context: context)) != nil else {
            return nil
        }
        return key
    }
    ...
    private static func clearKey(for alias: String) {
        let query = [kSecClass: kSecClassGenericPassword,
                     kSecAttrLabel: alias,
                     kSecAttrAccount: alias] as [String: Any]
